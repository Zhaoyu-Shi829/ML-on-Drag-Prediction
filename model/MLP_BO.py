...
# set up n_call=no_eval, random_state=no_sd(split dataset), epochs=no_epoch, no_stats=1~19
no_eval = 60
no_sd = 42
no_epoch = 300
no_stats = 10
...
# flag to choose NORM approach
minmax_norm = False
std_norm = True
# save directory of BO hyperparams, predictions, loss
cls = -1
base_dir = './Copy_Data/new_data_five_types/10primary_stats'
tags = ['gaus', 'pos', 'neg', 'Ex', 'Ez', 'hyb']

save_BO_dir = './tmp_mlp_BO/last_test_nn3/{}_sd{}_n{}_BOhp_stat{}_std_nl{}.csv'.format(tags[cls],
                                                                               no_sd, no_eval, no_stats, no_layer)
save_BOloss_dir = './tmp_mlp_BO/last_test_nn3/{}_sd{}_n{}_BOloss_stat{}_std_nl{}.csv'.format(tags[cls],
                                                                                no_sd, no_eval, no_stats, no_layer)
save_loss_dir = './tmp_mlp_BO/last_test_nn3/{}_sd{}_n{}_loss_pred_stat{}_std_nl{}.jpeg'.format(tags[cls],
                                                                                no_sd, no_eval, no_stats, no_layer)
save_pred_dir = './tmp_mlp_BO/last_test_nn3/{}_sd{}_n{}_pred_dns_stat{}_std_nl{}.csv'.format(tags[cls],
                                                                                no_sd, no_eval, no_stats, no_layer)
...
# CHOOSE NORMALIZATION (U+ is unncessary to norm I think)
def norm_stats():
    if minmax_norm:
        # MINMAX[-1,1]
...
# scaler.fit(train_one)
# mean, std = scaler.mean_, np.sqrt(scaler.var_)
# train = scaler.transform(train_one)
# x_train, y_train = train[:, :no_stats], train[:, -1]
# val = scaler.transform(val_one)
# x_val, y_val = val[:, :no_stats], val[:, -1]
# test = scaler.transform(test_one)
# x_test, y_test = test[:, :no_stats], test_one.iloc[:, -1]
